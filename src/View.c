...
View View_Invert(const View* view) {
    double oneOverSx = 1.0 / view->scale.x;
    double oneOverSy = 1.0 / view->scale.y;
    View viewInv;
    viewInv.offset = (Vector) {
        -view->offset.x * oneOverSx,
        -view->offset.y * oneOverSy
    };
    viewInv.scale = (Vector) {
        oneOverSx,
        oneOverSy
    };
    return viewInv;
